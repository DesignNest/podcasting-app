...
export default function Home() {
  ...
  const [socket] = useState<Socket>(() =>
    io(backendURL ? backendURL : "http://localhost:4000")
  );
  ...
  useEffect(() => {
    const getEmail = async () => {
      ...
      try {
        ...
        if (data.success) {
          const token = jwtDecode<TokenPayload>(data.message.value);
    ...
    const getToken = async () => {
      try {
        ...
        if (data.success) {
          const token = jwtDecode<MeetingDetails>(data.message.value);
  ...
  useEffect(() => {
    if (!meetingDetails?.username || !email) return;
    socket.emit("getUsers", {
      username: meetingDetails.username,
      myEmail: email,
    });
  }, [socket, meetingDetails?.username, email]);
